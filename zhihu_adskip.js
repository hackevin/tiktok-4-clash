const scriptName = "哲也同学",
  blockedUsersKey = "zhihu_blocked_users",
  currentUserInfoKey = "zhihu_current_userinfo",
  keywordBlockKey = "zhihu_keyword_block",
  blackAnswersIdKey = "zhihu_black_answers",
  zheyeServerKey = "zheye_server_url",
  defaultAnswerBlockedUsers = ["会员推荐", "盐选推荐"],
  keywordMaxCount = 1e3,
  $ = MagicJS("哲也同学", "INFO"),
  testUrl = (e) => new RegExp(e).test($.request.url);
function getUserInfo() {
  const e = { id: "default", is_vip: !1 };
  try {
    const { id: t = e.id, is_vip: r = e.is_vip } = $.data.read(
      currentUserInfoKey,
      {},
    );
    return { id: t, is_vip: r };
  } catch (t) {
    return ($.logger.error(`获取用户信息出现异常：${t}`), e);
  }
}
...
function unlockBlockedKeywords() {
  const e = (e, t) => {
    ...
    return $.env.isQuanX
      ? { body: e, headers: r, status: t }
      : {
          response: {
            body: e,
            headers: r,
            status: { "HTTP/1.1 200 OK": 200, "HTTP/1.1 400 Bad Request": 400 }[
              t
            ],
          },
        };
  ...
  try {
    ...
    let n = decodeURIComponent($.request.body).match(/keyword=(.*)/),
      o = n ? n[1] : "";
    if (
      ($.logger.debug(`准备操作本地关键词：${o}`), "GET" === $.request.method)
    ) {
    ...
    if ("POST" === $.request.method) {
      if (r.includes(o)) {
        return e(
          JSON.stringify({ error: { message: "关键词已存在", code: 100002 } }),
          "HTTP/1.1 400 Bad Request",
        );
      }
      (r.push(o), $.data.write(keywordBlockKey, r, t.id));
      const n = JSON.stringify({ success: !0 });
      return (
        $.logger.debug(`添加本地脚本屏蔽关键词“${o}”`),
        e(n, "HTTP/1.1 200 OK")
      );
    }
    if ("DELETE" === $.request.method) {
      ((n = decodeURIComponent($.request.url).match(/keyword=(.*)/)),
        (o = n ? n[1] : ""));
      const s = r.filter((e) => e !== o);
      $.data.write(keywordBlockKey, s, t.id);
      const i = JSON.stringify({ success: !0 });
      return (
        $.logger.debug(`删除本地脚本屏蔽关键词：“${o}”`),
        e(i, "HTTP/1.1 200 OK")
      );
    }
  } catch (e) {
    $.logger.error(`关键词屏蔽操作出现异常：${e}`);
  }
}
...
function manageBlackUser() {
  const e = getUserInfo();
  let t = {},
    r = $.data.read(blockedUsersKey, "", e.id);
  ((r = "object" == typeof r ? r : {}),
    defaultAnswerBlockedUsers.forEach((e) => {
      r[e] = t[e] = "00000000000000000000000000000000";
    }),
    $.logger.debug(`当前用户id：${e.id}，脚本黑名单：${JSON.stringify(r)}`));
  try {
    if ("GET" === $.request.method) {
      ...
      (n?.data?.forEach(({ name: e, id: t }) => {
        "[已重置]" !== e && e && t && (r[e] = t);
      }),
        $.data.write(blockedUsersKey, r, e.id),
        n?.paging?.is_end &&
          ($.notification.post(
            `同步黑名单数据成功！当前黑名单共${Object.keys(r).length - defaultAnswerBlockedUsers.length}人。\n脚本内置黑名单${defaultAnswerBlockedUsers.length}人。`,
          ),
          $.logger.debug(`脚本黑名单内容：${JSON.stringify(r)}。`)));
    } else if ("POST" === $.request.method) {
      let { name: t, id: n } = JSON.parse($.response.body);
      t &&
        n &&
        ((r[t] = n),
        $.data.write(blockedUsersKey, r, e.id),
        $.logger.debug(
          `${t}写入脚本黑名单成功，当前脚本黑名单数据：${JSON.stringify(r)}`,
        ),
        $.notification.post(`已将用户“${t}”写入脚本黑名单。`));
          ...
          for (let n in r)
            if (r[n] === t) {
              (delete r[n],
                $.data.write(blockedUsersKey, r, e.id),
                $.logger.debug(
                  `${n}移出脚本黑名单成功，当前脚本黑名单数据：${JSON.stringify(r)}`,
                ),
                $.notification.post(`已将用户“${n}”移出脚本黑名单！`));
              break;
            }
      }
    }
  } catch (e) {
    ($.logger.error(`操作黑名单失败，异常信息：${e}`),
      $.notification.post("操作黑名单失败，执行异常，请查阅日志。"));
  }
}
function autoInsertBlackList() {
  try {
    if (!1 === $.data.read("zhihu_settings_blocked_users", !0)) return null;
    const e = JSON.parse($.response.body);
    if (e.name && e.id && !0 === e.is_blocking) {
      const t = getUserInfo();
      ...
      ((r = ("object" == typeof r && r) ?? {}),
        r[e.name] ||
          ...
          (r[e.name] = e.id),
          $.data.write(blockedUsersKey, r, t.id),
          $.logger.debug(
            `${e.name}写入脚本黑名单成功，当前脚本黑名单数据：${JSON.stringify(r)}`,
          ),
          $.notification.post(`已自动将用户“${e.name}”写入脚本黑名单。`)));
    }
...
async function _setContentTagByCloud(e, t) {
  ...
  if (r) {
    ...
    const n = `${r}/api/v1.1/answer/links`;
    $.logger.debug(`服务端地址\n${n}`);
    const o = { links: e, custom_tags: getAllTagConfigs() };
    await $.http
      .post({
        url: n,
      ...
      .then((e) => {
        $.logger.debug(
          `云端探测结果<${typeof e.body}>\n${JSON.stringify(e.body)}`,
        );
        for (let r = 0; r < e.body.length; r++)
          try {
            let n = e.body[r];
            "" !== n && _setRecommendTag(t[r], n);
          } catch (e) {
            $.logger.error(e);
          }
      })
...
async function _setContentTagByLocal(e, t) {
  $.logger.debug(`将在本地请求以下回答：${e.join(",")}`);
  const r = getAllTagConfigs();
  function n(t) {
    return new Promise((n) => {
      (e[t] || n(""),
        $.logger.debug(`本地请求回答内容：${e[t]}`),
        $.http
          .get({ url: e[t], timeout: 1e3 })
          .then((e) => {
            let t = !1;
            $.logger.debug(`检测标签配置:\n${JSON.stringify(r)}`);
            for (let [o, s] of Object.entries(r))
              if (
                ($.logger.debug(`检测内容：${o}，正则：${s}`),
                "string" != typeof s && (s = String(s) || ""),
                s && new RegExp(s).test(e.body))
              ) {
                (n(o), (t = !0));
                break;
              }
  ...
  let o = [];
  for (let t = 0; t < e.length; t++) o.push(n(t));
  await Promise.all(o).then((e) => {
    $.logger.info(`本地探测结果<${e.length}>\n${JSON.stringify(e)}`);
    for (let r = 0; r < e.length; r++)
      try {
        let n = e[r];
        "" !== n && _setRecommendTag(t.data[r], n);
      } catch (e) {
        $.logger.error(e);
      }
  });
...
async function removeRecommend() {
  ...
  try {
    const e = {
        recommend_stream: $.data.read("zhihu_settings_recommend_stream", !1),
...
function removeQuestions() {
  try {
    ...
    const t = getUserInfo();
    let r = $.data.read(blockedUsersKey, "", t.id) || {};
    const n = $.data.read("zhihu_settings_blocked_users", !1);
    $.logger.debug(`当前黑名单列表: ${JSON.stringify(r)}`);
    let o = $.data.read(blackAnswersIdKey, []) || [];
    (e?.data &&
      (e.data = e.data.filter((e) => {
        const {
            target: t,
            target: { id: s = "", author: { name: i = "" } = {} } = {},
          } = e,
          a = r.hasOwnProperty(i),
          d = n && a;
        return (
          t &&
            ((e.target.visible_only_to_author = !1),
            (e.target.is_visible = !0),
            (e.target.is_copyable = !0)),
          d &&
            !o.includes(s.toString()) &&
            (o.push(s.toString()),
            $.notification.debug(`记录黑名单用户${i}的回答Id:${s}`)),
          !d
        );
      })),
      $.data.write(blackAnswersIdKey, o));
    const s = JSON.stringify(e);
    return ($.logger.debug(`修改后的回答列表数据：${s}`), { body: s });
  } catch (e) {
...
function insertContentTip() {
  ...
  if (e?.endorsement) {
    for (const [r, n] of Object.entries(t))
      if (
        ($.logger.debug(`检测内容：${n}，标签：${r}`),
        "string" == typeof n && new RegExp(n).test($.response.body))
      ) {
        $.logger.debug(`内容：\n ${$.response.body}\n匹配到标签：\n${r}`);
        const t = {
          action_url:
        ...
        e.endorsement.unshift(t);
        break;
      }
  } else if (e?.content_card_list)
    for (const [r, n] of Object.entries(t))
      if (
        ($.logger.debug(`检测内容：${n}，标签：${r}`),
        "string" == typeof n && new RegExp(n).test($.response.body))
...
function removeComment() {
  try {
    ...
    if (!e) return null;
    let t = JSON.parse(e);
    if (((t.ad_info = {}), !$.data.read("zhihu_settings_blocked_users", !1)))
      return { body: JSON.stringify(t) };
    const { id: r } = getUserInfo(),
      n = $.data.read(blockedUsersKey, {}, r) || {},
      o = (e) => {
        ((e.is_delete = !0),
          (e.can_reply = !1),
          (e.can_like = !1),
      ...
      i = (e) =>
        e.map((e) => {
          const { name: t } = e.author,
            r = e.reply_to_author?.name || "",
            a = n[t],
            d = n[r];
          return (
            (a || d) && (a && o(e), d && s(e.reply_to_author)),
            e.child_comments && (e.child_comments = i(e.child_comments)),
            e
          );
        });
    if (t.root) {
      const { name: e } = t.root.author;
      n[e] && o(t.root);
    }
    return (t.data && (t.data = i(t.data)), { body: JSON.stringify(t) });
  } catch (e) {
...
function modifyAnswersNextRender() {
  try {
    ...
    const t = JSON.parse(e),
      r = getUserInfo(),
      n = $.data.read("zhihu_settings_blocked_users", !1);
    let o = {};
    return (
      n &&
        ((o = $.data.read(blockedUsersKey, {}, r.id) || {}),
        $.logger.debug(`脚本黑名单用户：\n${JSON.stringify(o)}`)),
      (t.data = t.data.filter((e) => {
        const {
            ad_info: t = {},
            biz_type_list: r = [],
            type: s = "",
          ...
          d =
            ...
            "ad" === s ||
            i,
          l = n && o.hasOwnProperty(a);
        return (
          $.logger.debug(`用户${a}是否在黑名单中：${l}`),
          !(d || (n && l))
        );
      })),
      { body: JSON.stringify(t) }
...
function removeAnswerOrArticleAd() {
  ...
  try {
    const e = insertContentTip();
    return (
      delete e.third_business,
      $.logger.debug(JSON.stringify(e)),
      { body: JSON.stringify(e) }
    );
  } catch (e) {
    return ($.logger.error(`移除回答页广告出现异常：${e}`), null);
  }
}
function AsyncFunction() {
  return "[object AsyncFunction]" === Object.prototype.toString.call(this);
}
const urlHandlers = {
  resp_handlers: {
...
async function handleRequest(e) {
  let t = null;
  for (const r in urlHandlers[e])
    if (
      ($.logger.debug(`当前处理的URL：${$.request.url}，匹配规则：${r}`),
      $.logger.debug(`匹配结果${testUrl(r)}`),
      testUrl(r))
    ) {
      $.logger.debug(`当前处理的URL：${$.request.url}，匹配规则：${r}`);
      const n = urlHandlers[e][r];
      t = n instanceof AsyncFunction ? await n() : n();
      break;
    }
  return t;
}
  ...
  return new (class {
    constructor(e, t) {
      if (
        ...
        (this.scriptName = e),
        (this.env = (() => {
          const e = "undefined" != typeof $loon,
            t = "undefined" != typeof $task,
            r = "undefined" != typeof module,
            n = "undefined" != typeof $httpClient && !e,
            o = "undefined" != typeof $storm,
            s =
              "undefined" != typeof $environment &&
              void 0 !== $environment["stash-build"],
            i = "undefined" != typeof importModule;
          return {
            isLoon: e,
            isQuanX: t,
            isNode: r,
            ...
            get language() {
              if (n || s) return $environment.language;
            },
            get version() {
              return n
            ...
            get system() {
              return n ? $environment.system : r ? process.platform : void 0;
            },
            get systemVersion() {
              if (o) return $storm.systemVersion;
            },
            get deviceName() {
              if (o) return $storm.deviceName;
            },
          };
        })()),
        (this.logger = ((e, t = "INFO") => {
          let r = t;
          const n = {
              SNIFFER: 6,
              DEBUG: 5,
        ...
        (this.http =
          "function" == typeof MagicHttp
            ? MagicHttp(this.env, this.logger)
            : void 0),
        (this.data =
          "function" == typeof MagicData
            ? MagicData(this.env, this.logger)
            : void 0),
        (this.notification =
          "function" == typeof MagicNotification
            ? MagicNotification(
                this.scriptName,
                this.env,
                this.logger,
    ...
    get response() {
      ...
      (this.logger.debug(`SCRIPT COMPLETED: ${t} S.`),
        "undefined" != typeof $done && $done(e));
    };
  })(e, t);
}
function MagicNotification(e, t, r, n) {
  let o = null,
    s = null;
  function i(t = e, i = "", a = "", d = "") {
    if (void 0 === n || void 0 === n.post)
      throw "Bark notification needs to import MagicHttp module.";
    let l = {
      url: o,
      headers: { "content-type": "application/json; charset=utf-8" },
      body: { title: t, body: i ? `${i}\n${a}` : a, device_key: s },
    };
    n.post(l).catch((e) => {
      r.error(`Bark notify error: ${e}`);
    });
  }
  return {
    post: function (n = e, a = "", d = "", l = "") {
      ((l = ((e) => {
        try {
          let r = {};
          if ("string" == typeof e)
            ...
            if (t.isLoon)
              ((r.openUrl = e["open-url"] ? e["open-url"] : ""),
                (r.mediaUrl = e["media-url"] ? e["media-url"] : ""));
            else if (t.isQuanX) r = e["open-url"] || e["media-url"] ? e : {};
            else if (t.isSurge) {
              let t = e["open-url"] || e.openUrl;
              r = t ? { url: t } : {};
            }
          return r;
        } catch (e) {
        ...
        1 === arguments.length && ((n = e), (a = ""), (d = arguments[0])),
        r.notify(
          `title:${n}\nsubTitle:${a}\nbody:${d}\noptions:${"object" == typeof l ? JSON.stringify(l) : l}`,
        ),
        t.isSurge
          ? $notification.post(n, a, d, l)
          : t.isLoon
            ? l
              ? $notification.post(n, a, d, l)
              : $notification.post(n, a, d)
            : t.isQuanX && $notify(n, a, d, l),
        o && s && i(n, a, d));
    },
    debug: function (t = e, n = "", o = "", s = "") {
      "DEBUG" === r.getLevel() &&
        (1 === arguments.length && ((t = e), (n = ""), (o = arguments[0])),
        this.post(t, n, o, s));
    },
    bark: i,
    setBark: (e) => {
      try {
        let t = e.replace(/\/+$/g, "");
        ((o = `${/^https?:\/\/([^/]*)/.exec(t)[0]}/push`),
          (s = /\/([^\/]+)\/?$/.exec(t)[1]));
      } catch (e) {
        r.error(`Bark url error: ${e}.`);
      }
    },
  };
}
function MagicData(e, t) {
  let r = { fs: void 0, data: {} };
  if (e.isNode) {
    r.fs = require("fs");
    try {
      r.fs.accessSync(
        "./magic.json",
        r.fs.constants.R_OK | r.fs.constants.W_OK,
      );
    } catch (e) {
      r.fs.writeFileSync("./magic.json", "{}", { encoding: "utf8" });
    }
    r.data = require("./magic.json");
  }
  const n = (e, t) => "object" != typeof t && e === t,
    o = (e) => "true" === e || ("false" !== e && (void 0 === e ? null : e)),
    s = (e, t, r, n) => {
      if (r)
        try {
          ("string" == typeof e && (e = JSON.parse(e)),
            (e = !0 === e.magic_session ? e[r] : null));
        } catch {
          e = null;
        }
      ...
      return (
        !1 === n && e && !0 === e.magic_session && (e = null),
        null == e && null != t && (e = t),
        (e = o(e))
      );
    },
    i = (e) => {
      ...
      return e instanceof Array || null == e || e != e || "boolean" == typeof e
        ? {}
        : e;
    },
    a = (e, t = null, n = "", o = !1, i = null) => {
      let a = i || r.data;
      return (
        a && void 0 !== a[e] && null !== a[e]
          ? (val = a[e])
          : (val = n ? {} : null),
        (val = s(val, t, n, o)),
        val
    ...
    d = (r, n = null, o = "", i = !1, d = null) => {
      let l = "";
      return (
        d || e.isNode
          ? (l = a(r, n, o, i, d))
          : (e.isSurgeLike
              ? (l = $persistentStore.read(r))
              : e.isQuanX && (l = $prefs.valueForKey(r)),
            (l = s(l, n, o, i))),
        t.debug(
          `READ DATA [${r}]${o ? `[${o}]` : ""} <${typeof l}>\n${JSON.stringify(l)}`,
        ),
        l
      );
    },
    l = (n, o, s = "", a = null) => {
      if (void 0 === o || o != o) return !1;
      e.isNode ||
      ...
      if (
        (a || e.isNode
          ? (d = ((e, t, n = "", o = null) => {
              ...
              if (((s = i(s)), n)) {
                let r = i(s[e]);
                ((r.magic_session = !0), (r[n] = t), (s[e] = r));
              } else s[e] = t;
              return (null !== o && (o = s), s);
            })(n, o, s, a))
          : s
            ? (e.isSurgeLike
                ? (d = $persistentStore.read(n) ? $persistentStore.read(n) : d)
                : e.isQuanX &&
                  (d = $prefs.valueForKey(n) ? $prefs.valueForKey(n) : d),
              (d = i(d)),
              (d.magic_session = !0),
              (d[s] = o))
            : (d = o),
        d && "object" == typeof d && (d = JSON.stringify(d, null, 4)),
        t.debug(
          `WRITE DATA [${n}]${s ? `[${s}]` : ""} <${typeof o}>\n${JSON.stringify(o)}`,
        ),
        !a)
      ) {
        if (e.isSurgeLike) return $persistentStore.write(d, n);
        if (e.isQuanX) return $prefs.setValueForKey(d, n);
        if (e.isNode)
          try {
            return (r.fs.writeFileSync("./magic.json", d), !0);
          } catch (e) {
            return (t.error(e), !1);
          }
      }
  ...
  return {
    ...
    del: (n, o = "", s = null) => {
      ...
      if (s || e.isNode)
        ((a = ((e, t, n) => {
          ...
          return (
            (o = i(o)),
            t ? ((obj = i(o[e])), delete obj[t], (o[e] = obj)) : delete o[e],
            n && (n = o),
            o
          );
        })(n, o, s)),
          s
            ? (s = a)
            : r.fs.writeFileSync("./magic.json", JSON.stringify(a, null, 4)));
      else if (o) {
        (e.isSurgeLike
          ? (a = $persistentStore.read(n))
          : e.isQuanX && (a = $prefs.valueForKey(n)),
          (a = i(a)),
          delete a[o]);
        const t = JSON.stringify(a, null, 4);
        l(n, t);
      } else {
        if (e.isStorm) return $persistentStore.remove(n);
        if (e.isSurgeLike) return $persistentStore.write(null, n);
        if (e.isQuanX) return $prefs.removeValueForKey(n);
      }
      t.debug(`DELETE KEY [${n}]${o ? `[${o}]` : ""}`);
    },
    update: (e, t, r, s = n, i = null) => {
      t = o(t);
      if (!0 === s(d(e, null, r, !1, i), t)) return !1;
      {
        const o = l(e, t, r, i);
        let a = d(e, null, r, !1, i);
        return s === n && "object" == typeof a ? o : s(t, a);
      }
    },
    allSessions: (e, r = null) => {
      let n = {},
        o = d(e, null, null, !0, r);
      return (
        (o = i(o)),
        !0 === o.magic_session && ((n = { ...o }), delete n.magic_session),
        t.debug(
          `READ ALL SESSIONS [${e}] <${typeof n}>\n${JSON.stringify(n, null, 4)}`,
        ),
        n
      );
    },
    allSessionNames: (e, r = null) => {
      let n = [],
        o = d(e, null, null, !0, r);
      return (
        (o = i(o)),
        (n =
          !0 !== o.magic_session
            ? []
            : Object.keys(o).filter((e) => "magic_session" !== e)),
        t.debug(
          `READ ALL SESSIONS [${e}] <${typeof n}>\n${JSON.stringify(n, null, 4)}`,
        ),
        n
    ...
    defaultValueComparator: n,
    convertToObject: i,
  };
}
function MagicHttp(e, t) {
  let r;
  if (e.isNode) {
  ...
  class n {
    constructor(e = !0) {
      ((this.handlers = []), (this.isRequest = e));
    }
    use(e, r, n) {
      return (
        "function" == typeof e && t.debug(`Register fulfilled ${e.name}`),
        "function" == typeof r && t.debug(`Register rejected ${r.name}`),
        this.handlers.push({
          fulfilled: e,
          rejected: r,
          synchronous:
            !(!n || "boolean" != typeof n.synchronous) && n.synchronous,
  ...
  function o(r) {
    let n = { ...r };
    if (n.params && !e.isNode) {
      let e = Object.keys(n.params)
        .map((e) => {
          const t = encodeURIComponent(e);
          return (
            (n.url = n.url.replace(new RegExp(`${e}=[^&]*`, "ig"), "")),
            (n.url = n.url.replace(new RegExp(`${t}=[^&]*`, "ig"), "")),
            `${t}=${encodeURIComponent(n.params[e])}`
          );
        })
        .join("&");
      (n.url.indexOf("?") < 0 && (n.url += "?"),
        /(&|\?)$/g.test(n.url) || (n.url += "&"),
        (n.url += e),
        delete n.params,
        t.debug(`Params to QueryString: ${n.url}`));
    }
    return n;
  }
  const s = (e, t = null) => {
    ...
    i = (e, r = null) => {
      if (e && e.status >= 400)
        return (
          t.debug(`Raise exception when status code is ${e.status}`),
          {
    ...
    a = { request: new n(), response: new n(!1) };
  let d = [],
    l = [],
    u = !0;
  function c(e) {
    return (
      (e = o(e)),
      t.debug(`HTTP ${e.method.toUpperCase()}:\n${JSON.stringify(e)}`),
      e
    );
  }
  function g(e) {
    try {
      ((e = e ? s(e) : e),
        t.sniffer(
          `HTTP ${e.config.method.toUpperCase()}:\n${JSON.stringify(e.config)}\nSTATUS CODE:\n${e.status}\nRESPONSE:\n${"object" == typeof e.body ? JSON.stringify(e.body) : e.body}`,
        ));
      const r = i(e);
      return r ? Promise.reject(r) : e;
    } catch (r) {
      return (t.error(r), e);
    }
  }
  const f = (n, f) => {
    ...
    ((f = ((r, n) => {
      let s =
        "object" == typeof n ? { headers: {}, ...n } : { url: n, headers: {} };
      if (
        (s.method || (s.method = r),
        (s = o(s)),
        !0 === s.rewrite &&
          (e.isSurge
            ? ((s.headers["X-Surge-Skip-Scripting"] = !1), delete s.rewrite)
            : e.isQuanX && ((s.hints = !1), delete s.rewrite)),
        e.isSurgeLike)
      ) {
        const e = s.headers["content-type"] || s.headers["Content-Type"];
        "GET" !== s.method &&
          e &&
          e.indexOf("application/json") >= 0 &&
          s.body instanceof Array &&
          ((s.body = JSON.stringify(s.body)),
          t.debug(`Convert Array object to String: ${s.body}`));
      } else
        e.isQuanX
          ? (s.hasOwnProperty("body") &&
            ...
            (s.method = r))
          : e.isNode &&
            ("POST" === r || "PUT" === r || "PATCH" === r || "DELETE" === r
              ? (s.data = s.data || s.body)
              : "GET" === r && (s.params = s.params || s.body),
            delete s.body);
      return s;
    })(n.toUpperCase(), f)),
      (p = e.isNode
        ...
        : e.isSurgeLike
          ? (e) =>
              new Promise((t, r) => {
                $httpClient[n.toLowerCase()](e, (n, o, i) => {
                  if (n) {
                    let t = {
                      name: n.name || n,
                      message: n.message || n,
          ...
          : (e) =>
              new Promise((t, r) => {
                $task
                  ...
                  .catch((t) => {
                    let n = {
                      name: t.message || t.error,
                      message: t.message || t.error,
                      stack: t.error,
                      config: e,
                      response: t.response ? s(t.response) : null,
                    };
                    r(n);
                  });
              })),
          ...
          ((d = []),
            ...
            a.request.forEach((t) => {
              ("function" == typeof t.runWhen && !1 === t.runWhen(e)) ||
                ((u = u && t.synchronous), d.unshift(t.fulfilled, t.rejected));
            }),
            a.response.forEach((e) => {
              l.push(e.fulfilled, e.rejected);
            }));
        } catch (e) {
          t.error(`Failed to register interceptors: ${e}.`);
        }
      })(f));
    const y = [c, void 0],
      $ = [g, void 0];
    if (u) {
      for (
        t.debug("Interceptors are executed in synchronous mode"),
        ...
        try {
          ("function" == typeof e &&
            t.debug(`Executing request fulfilled ${e.name}`),
            (f = e(f)));
        } catch (e) {
          ("function" == typeof r &&
            t.debug(`Executing request rejected ${r.name}`),
            r(e));
          break;
        }
      }
      try {
        h = !e.isNode && f.timeout ? m(f) : p(f);
      } catch (e) {
        return Promise.reject(e);
      }
      for (Array.prototype.unshift.apply(l, $); l.length; )
        h = h.then(l.shift(), l.shift());
      return h;
    }
    {
      t.debug("Interceptors are executed in asynchronous mode");
      let r = [p, void 0];
      for (
        Array.prototype.unshift.apply(r, y),
        ...
        try {
          let n = r.shift(),
            o = r.shift();
          (!e.isNode && f.timeout && n === p && (n = m),
            "function" == typeof n &&
              t.debug(`Executing request fulfilled ${n.name}`),
            "function" == typeof o &&
              t.debug(`Executing request rejected ${o.name}`),
            (h = h.then(n, o)));
        } catch (e) {
          t.error(`request exception: ${e}`);
        }
      return h;
    }
    function m(e) {
      try {
        const t = new Promise((t, r) => {
          setTimeout(() => {
            let t = {
              message: `timeout of ${e.timeout}ms exceeded.`,
              config: e,
            };
            r(t);
          }, e.timeout);
        });
        return Promise.race([p(e), t]);
      } catch (e) {
        t.error(`Request Timeout exception: ${e}.`);
      }
    }
  };
  return {
    request: f,
    interceptors: a,
    convertHeadersToLowerCase: (e) =>
      Object.keys(e).reduce((t, r) => ((t[r.toLowerCase()] = e[r]), t), {}),
    convertHeadersToCamelCase: (e) =>
    ...
    delete: (e) => f("DELETE", e),
    head: (e) => f("HEAD", e),
    options: (e) => f("OPTIONS", e),
  };
}
(async () => {
  let e = null;
  ($.isResponse
    ? (e = await handleRequest("resp_handlers"))
    : $.isRequest
      ? (e = await handleRequest("req_handlers"))
